...
void AppendFileExtensionIfNeeded(const char* inFileName, const char *inExtensionWithDot, char** outFileName)
{
  *outFileName = 0;
  ...
  if (!extension)
  {
    ...
    tmp = new char[strlen(inFileName) + strlen(inExtensionWithDot) + 1];
    strcpy(tmp, inFileName);
    strcat(tmp, inExtensionWithDot);
    *outFileName = tmp;
    return;
  }
  ...
  tmp = new char[charsToCopy + strlen(inExtensionWithDot) + 1];
  strncpy(tmp, inFileName, charsToCopy);
  tmp[charsToCopy] = '\0';
  strcat(tmp, inExtensionWithDot);
  *outFileName = tmp;
